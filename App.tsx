...
import { saveSession, loadSession, clearSession, loadApiKeys, saveApiKeys, loadActiveKeyId, saveActiveKeyId } from './services/storageService';
...
const App: React.FC = () => {
  ...
  const handleFileSelect = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onload = (e) => {
        ...
        console.log(`[App] File selected for attachment: ${file.name}, size: ${file.size} bytes.`);
      };
  ...
  const handleSendMessage = async () => {
    ...
    const activeKeyDef = apiKeys.find(k => k.id === activeKeyId);
    console.log(`[App] Sending message to Eve (Tier: ${modelTier}, Mode: ${currentMode ? 'Generative' : 'Chat'}, HasAttachment: ${!!currentAttachment}).`);
    ...
    const handleStream = (streamedText: string) => {
        if (!streamStarted) {
            streamStarted = true;
            setIsThinking(false); // Stop thinking animation immediately on first token
        }
        ...
        setMessages(prev => {
            ...
            if (exists) {
                ...
                return prev.map(m => m.id === messageId ? { ...m, text: streamedText } : m);
            } else {
                // Create new message if it doesn't exist yet (should happen on first chunk)
                return [...prev, {
                    id: messageId,
                    role: 'model',
                    text: streamedText,
    ...
    try {
      ...
      // Final message update (important for non-streamed responses like images, and for final tag cleanup)
      setMessages((prev) => {
        const exists = prev.find(m => m.id === messageId);
        const finalMsg: Message = {
            id: messageId,
            role: 'model',
            text: response.text, // Final clean text
            image: response.image,
            isImageLoading: !!response.visualPrompt // Set true if a background generation is needed
        };

        if (exists) {
            return prev.map(m => m.id === messageId ? finalMsg : m);
        } else {
      ...
      if (response.visualPrompt) {
          console.log(`[App] Triggering background visual generation: ${response.visualPrompt}`);
          generateVisualSelfie(response.visualPrompt, modelTier, activeKeyDef?.key)
            .then((generatedImage) => {
                 if (generatedImage) {
      ...
      console.error("[App] Error sending message to Eve:", error);
      ...
      // If streaming started, we might want to append error or replace? 
      // For simplicity, just add error message at end.
      setMessages((prev) => [...prev, errorMsg]);
      setCurrentEmotion('neutral');
      console.log("[App] Error message added to state.");
    } finally {
      setIsThinking(false);
      console.log("[App] Finished thinking state.");
    }
  };
  ...
  const SidebarContent = () => (
    <>
      ...
      <div className="bg-slate-950/50 rounded-lg p-3 border border-slate-800 text-xs text-slate-400 shadow-inner space-y-2">
         ...
         >
            <span className="font-bold uppercase tracking-wide">API Keys</span>
            <span className="text-[10px]">{showKeyManager ? '▼' : '▶'}</span>
         </div>
           ...
           <div className="animate-fade-in space-y-3 pt-2">
             ...
             <div className="border-t border-slate-800 pt-2 space-y-2">
                <input 
                  ...
                  className="w-full bg-slate-900 border border-slate-700 rounded px-2 py-1 text-[10px] text-white focus:border-fuchsia-500 outline-none"
                />
                <input 
                  type="password" 
                  ...
                  className="w-full bg-slate-900 border border-slate-700 rounded px-2 py-1 text-[10px] text-white focus:border-fuchsia-500 outline-none font-mono"
                />
                <button 
                  onClick={handleAddKey}
                  disabled={!newKeyLabel.trim() || !newKeyValue.trim()}
                  className="w-full py-1 bg-slate-800 hover:bg-fuchsia-900 text-slate-400 hover:text-fuchsia-400 rounded text-[10px] transition-colors disabled:opacity-50"
                >
                  Save Key
                </button>
             </div>
        ...
        <div className="relative p-4 md:p-8 border-t border-slate-800 bg-slate-900/90 backdrop-blur-xl shadow-inner z-30">
          ...
          <div className="flex items-end gap-3 md:gap-4">
            ...
            <textarea
              ...
              placeholder={isThinking ? "EVE is thinking..." : "Ask EVE anything..."}
